...
use App\Filament\Actions\Exports\Jobs\CreatePdfFile;
use App\Filament\Actions\Exports\Jobs\CreateXlsxFile;
use App\Filament\Actions\Exports\Jobs\ExportCompletion;
use App\Filament\Actions\Exports\Jobs\PrepareCsvExport;
...
use App\Filament\Actions\Tables\ExportPdfAction as ExportTablePdfAction;
...
trait CanExportRecords
{
    ...
    protected function setUp(): void
    {
        ...
        $this->label(fn (ExportAction |ExportTablePdfAction |ExportTableBulkAction | ExportTableAction $action): string => __('filament-actions::export.label', ['label' => $action->getPluralModelLabel()]));

        $this->modalHeading(fn (ExportAction |ExportTablePdfAction |ExportTableBulkAction | ExportTableAction $action): string => __('filament-actions::export.modal.heading', ['label' => $action->getPluralModelLabel()]));
        ...
        $this->form(fn (ExportAction |ExportTablePdfAction |ExportTableBulkAction | ExportTableAction  $action): array => [
        ...
        $this->action(function (
            ExportAction | ExportTablePdfAction | ExportTableBulkAction | ExportTableAction $action,
            array $data,
            Component $livewire
        ) {
            ...
            $hasPdf = in_array(ExportFormat::Pdf, $formats);
            ...
            $makeCreatePdfFileJob = fn (): CreatePdfFile => app(CreatePdfFile::class, [
                'export' => $export,
                'columnMap' => $columnMap,
                'options' => $options,
            ]);
            ...
            Bus::chain([
                ...
                ...(($hasXlsx && $hasCsv && $hasPdf) ? [$makeCreatePdfFileJob()] : []),
            ])
                ->when(
                    filled($jobQueue),
        ...
        if (! $this instanceof ExportTableBulkAction) {
            $this->model(fn (ExportAction |ExportTablePdfAction |ExportTableBulkAction | ExportTableAction  $action): string => $action->getExporter()::getModel());
        }
    }
